...
19. **Deploy Realizado com Sucesso no GitHub (`repositoriocpd4-collab/2026Ficai`):**
    - Repositório remoto sincronizado na branch `main`.

20. **Modal 'Informações da FICAI' com Seções Colapsadas por Padrão:**
    - Os 5 blocos (`Situação do Aluno`, `Situações de Vulnerabilidade`, `Observação Inicial`, `Procedimentos Realizados` e `Diagnósticos da Evasão`) iniciam recolhidos (`collapsed`) ao abrir o modal, expandindo de forma fluida quando o usuário clica nos títulos.

21. **Infraestrutura Completa de Documentos Anexados no Supabase:**
    - **Tabela `ficai_info_entries` Expandida:** Inclusão das colunas `entry_kind`, `attachments` (JSONB) e `is_devolutiva`.
    - **Nova Tabela `ficai_documentos`:** Tabela relacional dedicada para arquivos anexados (Ofícios, Relatórios, Termos, Declarações, etc.), com campos para metadados, extensão, tipo mime, tamanho, URL e caminho de storage.
    - **Bucket de Storage `ficai-documentos`:** Configurado no script `supabase_schema.sql` com políticas RLS públicas para upload, leitura e exclusão.
    - **`DocumentService` Integrado:** Módulo em `supabase_client.js` para upload direto ao bucket e registro de documentos.
    - **Sincronização Bidirecional em `index.html`:** O envio de arquivos e anotações no "Histórico de acompanhamento após a geração" salva no IndexedDB local e sincroniza automaticamente com o Supabase Cloud.

22. **Limpeza e Zeramento das FICAIs Iniciais (Pronto para Teste Real):**
...
25. **Modal de Sucesso Estiloso (Salvar / Encaminhar ao CT / Imprimir PDF):**
    - Criado componente de modal de sucesso com design escuro em gradiente, partículas/brilho e animações pop fluidas.
    - Feedback customizado para cada ação:
      - **Salvar FICAI**: gradiente esmeralda, ícone ✅ e atalho para abrir o modal de informações.
      - **Encaminhar ao CT**: gradiente roxo, ícone ⚖️ e status de encaminhamento.
      - **Imprimir / Salvar PDF**: gradiente azul, ícone 🖨️ e confirmação de disparo da impressão.
    - **Commit `b5ea26a` implantado na branch `DevFicai`**.

26. **Reposicionamento do Mapa no Modal de Informações da FICAI:**
    - Mapa de rota interativo (OpenFreeMap / MapLibre GL) remanejado para a coluna esquerda do modal, logo abaixo de *Procedimentos realizados*, melhorando a proporção visual e o espaço disponível para a timeline à direita.
    - **Commit `40fcf11` implantado na branch `DevFicai`**.

27. **Seções Colapsáveis (Exibir / Ocultar) no Modal de Informações da FICAI:**
    - Suporte a clique para exibir e ocultar nas 5 seções do modal: *Situação do aluno*, *Situações de vulnerabilidade*, *Observação inicial*, *Procedimentos realizados* e *Diagnósticos da evasão*.
    - Adicionados ícones de chevron animados, feedback visual de hover, suporte a acessibilidade pelo teclado (Enter/Espaço) e recálculo dinâmico do viewport do mapa.
    - **Commit `0623ce1` implantado na branch `DevFicai`**.
